...
import table from './example-4';
const companies = createAll();

cleanConsole(5, companies);
console.log('---- EXAMPLE 5 --- ', 'Put here your function', table);

const example5 =(t)=>{
  const object = {size: null, average: null, hasCar: null, averageWithCar: null};
  object.size=t.length;
  let sum = t.map((x)=>{
    return x.age;
  });
  sum = sum.reduce((previous, current) => current += previous);
  object.average = sum / t.length;
  const hasCar = t.filter((x) => x.car===true);
  const sum2 = hasCar.map((x)=>{
    return x.age;
  });
  const sumUsersCars = sum2.reduce((previous, current) => current += previous);
  object.averageWithCar = sumUsersCars/hasCar.length;
  object.hasCar = hasCar.length;

  return object;
};

console.log(example5(table));
